...
class MongoController extends Controller implements IPersistentController {
    ...
    protected function getDb() {
        if (!isset($this->_db)){
            ...
            if(isset($client)){
                $this->_db =$client->selectDB ($this->getConfiguration()->get_MONGO_DB_NAME());
            }
        }
        return $this->_db;
    }
    ...
    public function getUser(){
        ...
        if(isset($id)){
            ...
            $db=$this->getDb();
            $usersCollection=$mongoConf->get_MONGO_USERS_COLLECTION();
            $usernameKey=$mongoConf->get_MONGO_USER_NAME_KEY_PATH();
            ...
            try {
                ...
                if (isset ( $user )) {
                    $passwordMatches=($user->{$mongoConf->get_MONGO_USER_PASSWORD_KEY_PATH()} === $this->_configuration->salt($password));
                    if (!$passwordMatches){
