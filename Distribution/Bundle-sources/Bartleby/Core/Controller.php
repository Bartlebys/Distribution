 ...
 */
class Controller implements IAuthentified,IAuthenticationControl{
    ...
    public function relayTrigger($senderUID,$collectionName,$origin,$homologousAction,$reference){
        ...
        if (isset($spaceUID) && isset($homologousAction) && isset($reference)){
            ...
            if (count($UIDS)>0){
                ...
                $trigger->index=$collection->count(['observationUID'=>$observationUID]);
                ...
                // We currently Encode manually.
                // Todo support Json encoding

                $date=new \DateTime();
                $iso8601=$date->format(DATE_ISO8601);
                $q=array(       MONGO_ID_KEY=>$trigger->UID,
