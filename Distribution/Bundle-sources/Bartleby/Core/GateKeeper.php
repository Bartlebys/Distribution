...
class GateKeeper implements IAuthentified{
    ...
    public function __construct(Configuration $configuration, $className, $methodName) {
        ...
        $this->_className = str_replace("\\Bartleby\\EndPoints\\", "", $className);
        $this->_className = str_replace("\\Bartleby\\Pages\\", "", $this->_className);
    ...
    private function _tokenIsValid($rule, $spaceUID) {
        if (Configuration::BY_PASS_SALTED_TOKENS) {
            return true;
    ...
    private function  _getDynamicPermissionForKey($key) {
        // TODO implement load from the collection
        return array();
    }
    ...
    protected function getDb() {
        if (!isset($this->_db)) {
            ...
            if (isset($client)) {
                $this->_db = $client->selectDB($this->_configuration->get_MONGO_DB_NAME());
            }
        }
        return $this->_db;
    }
