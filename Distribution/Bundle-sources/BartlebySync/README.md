...
Define the temp CREATIVE_KEY variable:

```shell
    CREATIVE_KEY=default-creative-key
```
...
```shell
    http -v -f POST ${BASE_URL}BartlebySync/install/ key=${CREATIVE_KEY}
````
...
```shell
    http -v -f POST  ${BASE_URL}BartlebySync/create/tree/1 key=${CREATIVE_KEY}	 http -v -f POST  ${BASE_URL}BartlebySync/create/tree/2 key=${CREATIVE_KEY}
```
...
### 5 touch the tree "1" to reset its public id, then try an unexisting ID
...
```shell
    http -v -f POST ${BASE_URL}BartlebySync/removeGhosts key=${CREATIVE_KEY}
