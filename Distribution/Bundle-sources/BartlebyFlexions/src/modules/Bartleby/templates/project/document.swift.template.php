...
@objc(<?php echo($configurator->getClassName()) ?>) public class <?php echo($configurator->getClassName())?> : <?php
...
foreach ($project->entities as $entity) {
    if ($configurator->actionsShouldBeSupportedForEntity($project,$entity)){
        ...
        echoIndentCR('dynamic lazy public var '.lcfirst($pluralizedEntity).'='.$collectionControllerClassName.'()',1);
    }
}
?>
...
foreach ($project->entities as $entity) {
    ...
    weak public var '.$arrayControllerVariableName.': NSArrayController?{

        willSet{
            // Remove observer on previous array Controller
            '.lcfirst($arrayControllerVariableName).'?.removeObserver(self, forKeyPath: "selectionIndexes", context: &_KVOContext)
        }
        didSet{
...
foreach ($project->entities as $entity) {
    if ($configurator->actionsShouldBeSupportedForEntity($project,$entity)){
        ...
        '.$entityDefinition.'.storage = BaseCollectionMetadatum.Storage.MonolithicFileStorage
...
// MARK: KVO
    ...
    <?php if ($isIncludeInBartlebysCommons==false){ echo("override");} ?> public func pushOperations() {
        self.pushOperations(self.operations.items)
    }


    <?php if ($isIncludeInBartlebysCommons==false){ echo("override");} ?> public func synchronize(){
    ...
    <?php if ($isIncludeInBartlebysCommons==false){ echo("override");} ?> public func optimizeOperations() {
