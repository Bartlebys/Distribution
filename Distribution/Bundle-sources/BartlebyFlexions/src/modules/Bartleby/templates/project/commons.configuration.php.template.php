    ...
    */
    ...
    private function _configureFilters(){
        ...
        $filterUpdateUser->closure=function($data) {
            ...
            KeyPath::setValueByReferenceForKeyPath($data,"user.password",$this->salt($password));
			// 2#
			// let's block attempts to update spaceUID
			// To block the spaceUID updates we will set the spacedUID to NULL
			// if we discover that it has been malicously udpated by the client.
			$client= new MongoClient ();
			if(isset($client)){
				$db =$client->selectDB ($this->get_MONGO_DB_NAME());
				$users = $db->users;
				try {
					$userUID = KeyPath::valueForKeyPath($data, "user._id");
					$q = array('_id' => $userUID);
					$r = array();
					$r = $users->findOne($q);
					if (isset($r)) {
						$spaceUID=KeyPath::valueForKeyPath($data,"user.spaceUID");
						if (array_key_exists('spaceUID',$r)){
							$savedUID=$r['spaceUID'];
							if ($savedUID == $spaceUID){
								return $data;
							}
						}
					}
