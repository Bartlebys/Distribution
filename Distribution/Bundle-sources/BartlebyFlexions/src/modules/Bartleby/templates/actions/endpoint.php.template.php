...
if($d->httpMethod=='POST') {
    ...
    function call(' . $callDataClassName . ' $parameters) {
        ...
        try {
            ...
            if ($e->getCode()==11000 && Configuration::IGNORE_MULTIPLE_CREATION_IN_CRUD_MODE ){
                // We return A 200 not a 201
                return new JsonResponse(\'This is not the first attempt. We use the quiet mode and do ignore Multiple Creation in CRUD mode\',200);
            }
            ':'').'
            return new JsonResponse( array (\'code\'=>$e->getCode(),
                                            \'message\'=>$e->getMessage(),
