...
while ( $d ->iterateOnProperties() === true ) {
   ...
   $mutable=($property->mutability=='variable' ? 'var ':'let ');
    ...
    if($property->type==FlexionsTypes::ENUM){
        ...
        echoIndentCR('public enum ' .$enumTypeName.':'.ucfirst(FlexionsSwiftLang::nativeTypeFor($property->instanceOf)). '{', 1);
        ...
        echoIndentCR('}', 1);
        echoIndentCR($prefix. $name .':'.$enumTypeName._propertyValueString($property), 1);
        ...
        echoIndentCR($prefix. $name .':['.ucfirst($instanceOf). ']'._propertyValueString($property), 1);
    }else if($property->type==FlexionsTypes::OBJECT){
        echoIndentCR($prefix. $name .':'.ucfirst($property->instanceOf)._propertyValueString($property), 1);
    }else{
        $nativeType=FlexionsSwiftLang::nativeTypeFor($property->type);
        if(strpos($nativeType,FlexionsTypes::NOT_SUPPORTED)===false){
            echoIndentCR($prefix. $name .':'.$nativeType._propertyValueString($property), 1);
        }else{
            echoIndentCR($prefix. $name .':Not_Supported = Not_Supported()//'. ucfirst($property->type), 1);
        }
    }
}
